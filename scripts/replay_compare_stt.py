...
def _build_report(
    *,
    ...
    parts.append("")
    parts.append(f"- **Flux (v2)** sends the keyterm list inside its `Configure` JSON; the")
    parts.append(f"  full n={len(keyterms)} list is delivered.")
    parts.append(f"- **Nova-3 (v1)** URL-encodes each keyterm as a repeated query parameter")
