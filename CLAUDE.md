...
**Scheduled jobs subsystem (`bot/jarvis/jobs/`):** the Jarvis bot runs APScheduler in-process and posts to the *right channel* based on signal type — `morning_sprint_brief` → `#weekly-sync`; `dependabot_pair_check` → `#code-review`; `stuck_in_progress` → `#blockers`; `end_of_week_recap` → `#milestones-updates`. `#jarvis` is the bot-meta channel — boot pings, per-job audit, failures. The original spec also defined three PR-nag kinds (`pr_review_nudge`, `approved_pr_not_merged`, `ci_red_pr_nudge`); they were removed pre-fire as too noisy for a small team — git history has them if reintroduced. See `docs/superpowers/specs/2026-05-06-jarvis-scheduled-jobs-design.md`. Trigger any job locally with `PYTHONPATH=bot .venv/Scripts/python -m jarvis.jobs.run <job-name>`.
